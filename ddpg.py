...
class DDPG(object):
    ...
    def sample_batch(self,epoch_inx = None):
        if self.bc_loss: #use demonstration buffer to sample as well if bc_loss flag is set TRUE
            ...
            transitions = self.buffer.sample(self.batch_size,epoch_inx = epoch_inx) #otherwise only sample from primary buffer
    ...
    def stage_batch(self, batch=None, epoch_inx=None):
        if batch is None:
            batch = self.sample_batch(epoch_inx = epoch_inx)
        assert len(self.buffer_ph_tf) == len(batch)
        self.sess.run(self.stage_op, feed_dict=dict(zip(self.buffer_ph_tf, batch)))

    def train(self, stage=True, epoch_inx = None):
        if stage:
            self.stage_batch(epoch_inx = epoch_inx)
